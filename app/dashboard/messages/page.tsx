...
export default function MessagesPage() {
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-3xl h-[calc(100vh-140px)] flex overflow-hidden shadow-xl shadow-slate-200/20">

      {/* SIDEBAR: CHATS & SEARCH LIST */}
      <div className={`w-full md:w-[340px] border-r border-slate-100 flex flex-col shrink-0 ${selectedChatId ? 'hidden md:flex' : 'flex'}`}>
        <div className="p-4 border-b border-slate-50 bg-white">
          <h1 className="text-2xl font-black text-slate-900 tracking-tight mb-3.5 flex items-center gap-2">
            Xabarlar
          ...
          <div className="relative flex items-center group">
            ...
            <input
              ...
              className="w-full bg-slate-50 text-xs font-semibold pl-10 pr-9 py-3 rounded-2xl border border-transparent focus:border-blue-500/20 focus:bg-white outline-none transition-all"
            />
            {isSearching && (
          ...
          {!userIdReady && (
            <p className="mt-2 text-[10px] font-bold text-amber-600 bg-amber-50 px-2.5 py-1.5 rounded-lg">
            ...
            chats.map((chat) => {
              ...
              return (
                <div
                  ...
                  className={`p-3.5 flex items-center gap-3.5 cursor-pointer rounded-2xl transition-all duration-200 active:scale-[0.98] ${isSelected ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/15' : 'hover:bg-slate-50 text-slate-800'
                    }`}
                >
                  <img src={partner.avatar_url || FALLBACK_AVATAR} alt="" className="w-11 h-11 object-cover rounded-full" />
                  <div className="flex-1 min-w-0">
                    <h4 className={`font-bold text-sm truncate ${isSelected ? 'text-white' : 'text-slate-900'}`}>{partner.nickname}</h4>
                    <p className={`text-xs truncate ${isSelected ? 'text-white/80' : 'text-slate-400'}`}>@{partner.username}</p>
                  </div>
                </div>
              )
            })
      ...
      {/* MAIN: CHAT WINDOW */}
      <div className={`flex-1 flex flex-col bg-slate-50/30 ${!selectedChatId ? 'hidden md:flex items-center justify-center text-center p-8' : 'flex'}`}>
        {activeChat && activeUser ? (
          ...
          >
            ...
            <div className="px-4 py-3 bg-white border-b border-slate-100 flex items-center justify-between shadow-xs">
              <div className="flex items-center gap-3 min-w-0">
                <button onClick={() => setSelectedChatId(null)} className="p-2 hover:bg-slate-50 rounded-xl md:hidden text-slate-600 active:scale-90 transition-transform">
                ...
                <div>
                  <h3 className="font-black text-sm text-slate-900 flex items-center gap-1">{activeUser.nickname} <HiCheckBadge className="w-4 h-4 text-blue-500" /></h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Muloqotda</p>
                </div>
              </div>
              <button className="p-2.5 hover:bg-slate-50 rounded-xl text-slate-500"><HiOutlineEllipsisVertical className="w-5 h-5" /></button>
            </div>
                    ...
                    <React.Fragment key={msg.id}>
                      ...
                      <div className={`flex flex-col max-w-[70%] mb-2 ${isMe ? 'self-end items-end' : 'self-start items-start'}`}>
                        ...
                        <motion.div
                          ...
                          className={`px-4 py-2.5 text-sm font-medium leading-relaxed shadow-xs ${isMe
            ...
            {/* MESSAGE INPUT FORM */}
            <form onSubmit={handleSendMessage} className="p-3.5 bg-white border-t border-slate-100 flex items-end gap-2.5">
              <input type="file" accept="image/*,video/*" ref={fileInputRef} onChange={handleFileChange} className="hidden" />

              <button type="button" onClick={() => fileInputRef.current?.click()} className="p-2.5 mb-0.5 text-slate-400 hover:text-slate-600 hover:bg-slate-50 rounded-xl active:scale-90 transition-all">
              ...
              <div className="relative flex-1 flex items-end group">
                <textarea
                  ...
                  className="w-full bg-slate-50 text-xs font-semibold pl-4 pr-10 py-3.5 rounded-2xl border border-transparent focus:border-blue-500/10 focus:bg-white outline-none transition-all duration-200 text-slate-800 resize-none max-h-[120px] [&::-webkit-scrollbar]:hidden"
                />
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.85 }}
                className={`p-3.5 rounded-2xl flex items-center justify-center transition-colors duration-300 ${typedMessage.trim() && !isUploading ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'bg-slate-100 text-slate-400 cursor-not-allowed'
                  }`}
              >
          ...
          <div className="flex flex-col items-center justify-center max-w-sm m-auto animate-in fade-in zoom-in-95">
            <div className="w-16 h-16 rounded-3xl bg-gradient-to-tr from-blue-50 to-indigo-50 flex items-center justify-center mb-4 shadow-inner">
            ...
            <h3 className="font-black text-base text-slate-900 mb-1.5">Muloqotni boshlang</h3>
            <p className="text-xs text-slate-400 font-bold leading-relaxed px-4 text-center">Xabarlar, rasm yoki videolarni realtime almashish uchun foydalanuvchini qidiring yoki chatni tanlang.</p>
          </div>
        )}
      </div>
